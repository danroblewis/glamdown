 ...
 */
import { getGrammarByName, createHighlightCache, highlightWithCache, } from 'blecsd';
...
function highlightLineAnsi(lineText, tokens, theme) {
    if (tokens.length === 0) {
    ...
    for (const token of tokens) {
        ...
        const color = colorKey ? theme[colorKey] : theme.codeText;
        result += sgrFg(color) + token.text;
    }
    result += sgrReset();
    return result;
}
export function renderCode(data, theme, width) {
    const lines = [];
    ...
    for (let i = 0; i < codeLines.length; i++) {
        ...
        const line = borderColor + '│' + sgrReset() + ' ' +
